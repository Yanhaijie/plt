...
public final class AdvanceAIUtils extends OpenApiClient{
    ...
    private static String apiIDCheck = "/openapi/anti-fraud/v3/identity-check";
    ...
    public static void main(String[] args){
        idCheck("Ellen Yapyteri","1271205802980001","972a20dd25fb6075","bad947e25dc9e1d4");
    }
     ...
     * @return
     */
    public static Result idCheck(String name,String idNumber,String accessKey, String secretKey){
        Result result = ResultGenerator.genFailResult("匹配失败");
        ...
        try {
            ...
            imagesMap.put("name", name);
            imagesMap.put("idNumber", idNumber);
            String response = client.request(apiIDCheck, JSONObject.toJSONString(imagesMap));
            Map<String,Object> responsMap = (Map<String,Object>)JSON.parse(response);
            System.out.println(responsMap.toString());
            if (responsMap.get("code").equals("SUCCESS")){
                ...
                result = ResultGenerator.genResult(ResultCode.INTERNAL_SERVER_ERROR.code ,ResultCode.INTERNAL_SERVER_ERROR.msg);
                logger.info("advance ai 欠费， 请及时充值");
            }
            else if (responsMap.get("code").equals("PERSON_NOT_FOUND")) {
                result = ResultGenerator.genResult(ResultCode.NIK_ERROR.code ,ResultCode.NIK_ERROR.msg);
            }
            else {
                result = ResultGenerator.genResult(ResultCode.INTERNAL_SERVER_ERROR.code ,ResultCode.INTERNAL_SERVER_ERROR.msg);
