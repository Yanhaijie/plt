...
import com.supermoney.loan.api.dao.SExchangeRateMapper;
...
public class SExchangeRateServiceImpl extends AbstractService<SExchangeRate> implements SExchangeRateService {
    ...
    public  SExchangeRate getIndonesiaExchange(){
        if(indonesiaExchange==null){
            indonesiaExchange=getUsdByCountry(Constants.Country.PHP_CR);
        }
    ...
    public String usdToIndoneslaFormat(BigDecimal amount) {
        return indonwslaFormat(usdToIndonesla(amount,false))+"PHP";
    }
    ...
    public BigDecimal valFormatCountry(SExchangeRate rate,BigDecimal amount){
        ...
        if(rate.getSecondCurrency().equals(Constants.Country.PHP_CR)){
